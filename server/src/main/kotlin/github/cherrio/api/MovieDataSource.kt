...
val movieDataSource by lazy { MovieDataSource(CIO.create()) }
class MovieDataSource(private val clientEngine: HttpClientEngine) {
    ...
    suspend fun getMovies(page: Int = 1) = coroutineScope {
        val hollywood = async { getMovie("videos/movies/page/$page") }
        val nollywood = async{ getMovie("videos/nollywood/page/$page") }
        val series = async { getMovie("videos/series/page/$page") }
        val yoruba = async{ getMovie("videos/yoruba/page/$page") }
    ...
    private suspend fun getMovie(url: String): List<Movie> {
        println("Started: with $url")
